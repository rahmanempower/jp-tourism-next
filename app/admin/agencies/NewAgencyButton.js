...
export default function NewAgencyButton() {
    ...
    async function submit() {
        ...
        const ownerFirstName = form.ownerFirstName.trim();
        const ownerLastName = form.ownerLastName.trim();
        const ownerPhone = form.ownerPhone.trim();

        if (!businessName || !contactEmail || !contactPhone || !licenseNumber) {
            toast.current?.show({ severity: "warn", summary: "Business name, contact email, phone and license number are required." });
            return;
        }

        if (!isValidEmail(contactEmail)) {
            toast.current?.show({ severity: "warn", summary: "Contact email is invalid." });
            return;
        }

        if (!isFiniteNumber(creditLimit) || creditLimit < 0) {
            toast.current?.show({ severity: "warn", summary: "Credit limit must be a valid non-negative number." });
            return;
        }

        if (!isFiniteNumber(marginPercent) || marginPercent < 0 || marginPercent > 100) {
            toast.current?.show({ severity: "warn", summary: "Margin % must be between 0 and 100." });
            return;
        }

        if (form.createOwner) {
            if (!ownerEmail || !form.ownerPassword || !ownerFirstName || !ownerLastName) {
                toast.current?.show({ severity: "warn", summary: "Owner email, password, first and last name are required." });
                return;
            }

            if (!isValidEmail(ownerEmail)) {
                toast.current?.show({ severity: "warn", summary: "Owner email is invalid." });
                return;
            }
        ...
        <>
            ...
            >
                <div className="p-fluid">
                    ...
                    >
                        <Button label="Cancel" outlined onClick={close} disabled={submitting} style={{
                            border: "1px",
                            borderRadius: 10,
                            padding: "0.62rem 0.95rem",
                            boxShadow: "0 8px 22px rgba(79, 70, 229, 0.28)",
